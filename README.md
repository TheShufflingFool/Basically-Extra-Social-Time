...
Mods for The Legend of Heroes: Trails of Cold Steel games that ensure it's possible to fully bond with all the characters, even on the first playthrough.
Tested on XSEED and NISA PC versions of the games.

For Cold Steel II, make sure to download [B.R.U.H.](https://github.com/TheShufflingFool/Black-Records-Unrestricted-Hunt) as well; otherwise, you might stumble upon an odd book at some point.
...
* updated the system messages to account for increased amounts of bonding points and tickets
...
* fixed the (most likely) unintended game behavior in CS2, where the characters would refer to CS1's final bonding event even when they weren't picked for it - no more seeing flashbacks of events that never happened or hearing about promises that were never made
...
2. Download the latest release of `BasicallyExtraSocialTime_CS(1/2/3/4).p3a` ([DOWNLOAD LINK](https://github.com/TheShufflingFool/Basically-Extra-Social-Time/releases/latest)).
...
5. Move or copy `BasicallyExtraSocialTime_CS(1/2/3/4).p3a` to the `mods` folder.
6. Delete `order.txt` (if it exists) from the `mods` folder.
   * if you have an older version of this mod named `ExtraSocialTime_CS(1/2/3/4).p3a`, you can delete it as well
8. To uninstall, delete `BasicallyExtraSocialTime_CS(1/2/3/4).p3a` from the `mods` folder.

### UNSUPPORTED METHOD
If, for some reason, you can't or don't want to use SenPatcher, follow these steps:
1. Install B.R.U.H. first if you intend to use it too.
2. Go to the latest release ([here](https://github.com/TheShufflingFool/Basically-Extra-Social-Time/releases/latest)) and download `Source code (zip)`.
3. Locate the game's main directory:
   * for Steam version, right-click on `The Legend of Heroes: Trails of Cold Steel (II/III/IV)` and select Manage → Browse local files
   * for GOG version, right-click on `The Legend of Heroes: Trails of Cold Steel (II/III/IV)` and select Manage installation → Show folder
4. Open the downloaded file, enter the folder for the game you want to mod and extract the `data` folder into the game's main directory (you can just drag and drop or copy and paste).
5. When prompted to merge folders and replace files, accept all changes.
6. To uninstall, verify integrity of game's files.
...
* **How do I know I installed the mod correctly?**  
  ...
  If you use other mods and B.E.S.T. doesn't work as intended, it's highly likely there is a compatibility issue. You can try editing `order.txt` inside the `mods` folder and moving `BasicallyExtraSocialTime_CS(1/2/3/4).p3a` to the top of the list. That should unlock the extra bonding points but might override changes from the other mods.
...
* **I have the mod but I'm still missing a few bonding points/tickets. Did I do something wrong?**  
  Unlike NG+, the mod may not give you the necessary amount right off the bat. You're still required to work for it, so go do some side content and you'll find what you need.
